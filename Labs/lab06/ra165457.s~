...
loop_c:
        ...
        cmp  r3, #'1'         @ compara r3 com '1'  (49 em ASCII)
...
@ -- multiplica o numero por 2
		...
		cmp r0, #1 				@vendo se N é 1, se for encerramos o programa e retornamos 1
		beq fim
		cmp r0, #2 				@vendo se N é 2, se for encerramos o programa e retornamos 1
		moveq r0, #1
		beq fim
		mov r1, #1 				@caso N > 2 colocamos em r1 e em r2 o valor dos 1os numeros de fribonaci e diminuimos N em 2 (primeiras iteracoes)
		mov r2, #1
		sub r0, #2

fribonacci:
		cmp r0, #0 				@executa o while enquanto N != 0
		moveq r0, r2 			@se N == 0 entao movemos o valor de r2 para r0 e saimos do loop
		beq fim
		add r3, r2, r1 			@caso N != 0 somamos os 2 numeros de fribonacci e guardamos em r3
		mov r1, r2 				@ movemos r2 para r1
		mov r2, r3 				@atualiza r2 para o valor da soma
		sub r0, #1 				@decrementamos N
		b fribonacci 			@vamos para o loop
